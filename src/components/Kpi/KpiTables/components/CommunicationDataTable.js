...
// const CommunicationDataTable = () => {
//   return (
//     <div>CommunicationDataTable</div>
//   )
// }

// export default CommunicationDataTable
...
const CommunicationDataTable = ({ category, subcategory }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/getCommunications/${category}`);
  ...
  const columns = [
    ...
    { field: 'kpi_name', headerName: 'Name', width: 300,
    renderCell: (params) => (
    ...
    {field: 'calculation', headerName:'Calculation' , width:400,
    renderCell: (params) => (
      <div style={{ whiteSpace: 'pre-line', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {params.value}
      </div>
    ),},
    {field: 'subfunction' ,  headerName:'Subfunctions',width:150,
    renderCell: (params) => (
      <div style={{ whiteSpace: 'pre-line', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {params.value}
      </div>
