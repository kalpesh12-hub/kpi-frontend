...
const PrmanagementTable = ({ category, subcategory }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/getProject/${category}`);
  ...
  const columns = [
    ...
    {field: 'calculation', headerName:'Calculation' , width:500,
    renderCell: (params) => (
      <div style={{ whiteSpace: 'pre-line', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {params.value}
      </div>
    ),},
    {field: 'subfunction' ,  headerName:'Subfunctions',width:300,
    renderCell: (params) => (
      <div style={{ whiteSpace: 'pre-line', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {params.value}
      </div>
...
export default PrmanagementTable;
