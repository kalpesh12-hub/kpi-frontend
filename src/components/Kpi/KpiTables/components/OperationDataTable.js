...
const OperationDataTable = ({ category, subcategory }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/getOperations/${category}`);
  ...
  const columns = [
    ...
    { field: 'kpi_name', headerName: 'Name', width: 200,
    renderCell: (params) => (
