...
const SalesDataTable = ({ category, subcategory }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/getSales/${category}`);
  ...
  const columns = [
    ...
    { field: 'func', headerName: 'Functions', width: 100,
    renderCell: (params) => (
      <div style={{ whiteSpace: 'pre-line', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {params.row.function}
      </div>
