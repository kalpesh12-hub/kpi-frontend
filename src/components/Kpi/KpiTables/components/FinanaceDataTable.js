...
const FinanaceDataTable = ({ category, subcategory }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/getFinance/${category}`);
  ...
  const columns = [
    { field: 'isr_no', headerName: 'ID', width: 70 },
...
export default FinanaceDataTable;
